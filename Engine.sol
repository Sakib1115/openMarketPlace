...
contract Engine is Ownable, ReentrancyGuard {
    ...
    event CancelAuctionAndPayBackLastBid(
        uint256 _index,
        address _bidder,
        uint256 amount
    );
    ...
    function buy(uint256 _tokenId) external payable nonReentrant {
        ...
        Fungy721 asset = Fungy721(offer.assetAddress);
    ...
    function getBiddingFundsBack(uint256 auctionIndex) public {
        Auction memory auction = auctions[auctionIndex];
        
        require(
            msg.sender == auction.currentBidOwner,
            "You are not the best bidder"
        );
        require(isFinished(auctionIndex), "The auction is still active");
        Offer memory offer = offers[auction.assetId];
        require(offer.isAuction == true, "NFT not in auction"); // to avoid calling this method two times
        // #4. Only if there is at least a bid and the bid amount > 0, give it back to last bidder
        require(
            auction.currentBidAmount != 0 && auction.bidCount > 0,
            "not a valid bid"
        );
        // return funds to the previuos bidder
        (bool success3, ) = auction.currentBidOwner.call{
        ...
        emit CancelAuctionAndPayBackLastBid(auctionIndex,
            auction.currentBidOwner,
            auction.currentBidAmount);
    ...
    function acceptBidAsWinner(uint256 auctionIndex) public {
        Auction storage auction = auctions[auctionIndex];
        require(
    ...
    function auctionTransferAsset(uint256 auctionIndex) private nonReentrant {
        ...
        Fungy721 asset = Fungy721(auction.assetAddress);
    ...
    function IAmAlive() public view returns (uint256) {
        return block.timestamp;
    }
}
